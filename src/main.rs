...
fn main() {
    ...
    let mut indata = vec![vec![0.0_f64; max_win_size]; num_channels]; //0 pads the front of the channel with a max_frame of 0s
                                                                      //let mut indata = vec![vec![0.0_f64; 0]; num_channels];
...
fn process_microframe(
    ...
    let mut flipped_frame = ifft.make_output_vec(); //vec![0.0; win_len];
    let mut spectrum_out: Vec<Complex<f64>> = fft.make_output_vec();
    ...
    for _count in 0..num_ffts {
        ...
        (0..spectrum.len()).for_each(|iter| {
            let mut temp = spectrum[iter].to_polar();
            temp.0 = temp.0 * filt_win[iter];
            temp.1 = rng.gen::<f64>() * PI - (PI / 2.0);
            spectrum_out[iter] = num_complex::Complex::from_polar(temp.0, temp.1);
        });

        //this is where it is failing in realfft version 3. not sure why. seems to not like random phase
        //performs the ifft
...
fn process_chunk(
    ...
    for big_iter in 0..stretch_points.len() {
        ...
        let mut part: Vec<f64> = fft.make_input_vec();
